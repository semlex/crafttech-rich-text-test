...
### Идеальный вариант:
1. Текст, текстовый редактор и картинка полностью совпадают. То есть для пользователя не должно быть различий (картинка это или html). Соответственно и поля для ввода заменяется так, как будто оно там всегда было
2. Текст не должен выходить за пределы фигуры
